...
logistic regression models the **log-odds of the probability** as a linear combination of input features and then maps it through a **sigmoid function** to ensure the output lies between 0 and 1. The model is trained by **minimizing the binary cross-entropy (log loss)** using gradient descent, and predictions are made by applying a threshold to the probability. It is widely used due to its simplicity, interpretability, and effectiveness in many classification tasks
